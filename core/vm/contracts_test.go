...
func testPrecompiled(addr string, test precompiledTest, t *testing.T) {
	...
	t.Run(fmt.Sprintf("%s-Gas=%d", test.Name, gas), func(t *testing.T) {
		if res, _, err := runPrecompiledContract(nil, p, AccountRef(common.Address{}), p.Address(), in, gas, new(big.Int), false); err != nil {
			t.Error(err)
...
func testPrecompiledOOG(addr string, test precompiledTest, t *testing.T) {
	...
	t.Run(fmt.Sprintf("%s-Gas=%d", test.Name, gas), func(t *testing.T) {
		_, _, err := runPrecompiledContract(nil, p, AccountRef(common.Address{}), p.Address(), in, gas, new(big.Int), false)
...
func benchmarkPrecompiled(addr string, test precompiledTest, bench *testing.B) {
	...
	bench.Run(fmt.Sprintf("%s-Gas=%d", test.Name, reqGas), func(bench *testing.B) {
		...
		for i := 0; i < bench.N; i++ {
			copy(data, in)
			res, _, err = runPrecompiledContract(nil, p, AccountRef(common.Address{}), p.Address(), in, reqGas, new(big.Int), false)
		}
		bench.StopTimer()
